...
- imagePrompt: concise image-generation prompt for one safe, documentary-style still preview, no logos, no copyrighted characters, no fake evidence
- transition: restrained transition into the next scene

Write each caption in the same language as its narration. Keep visual, searchQuery, imagePrompt, and transition production directions in English so production tools receive consistent instructions. Do not output IDs, scene numbers, start/end times, durations, narration, script text, provider names, URLs, licensing claims, or legal conclusions. Do not rewrite or summarize the narration. Search queries are proposals only; never state that footage is available, cleared, or licensed. Follow the visual constraints while avoiding graphic, deceptive, or unsupported imagery.`;

...
export function renderNewProject(draft = {}, error = null) {
  return `${pageHeading("New production", "Define the story only you should make.", "CreatorPilot fetches and compares several references' storytelling mechanics, researches your exact angle, then writes from a source-grounded creative brief.")}
    ...
    <form class="reference-form" id="reference-form" novalidate>
      <div class="form-primary">
        <section class="form-section reference-set-section" aria-labelledby="reference-heading">
          <div class="section-index">01</div><div><h2 id="reference-heading">Reference set</h2><p>Add three YouTube URLs. Two more can strengthen the comparison.</p></div>
          <div class="field-span reference-set-fields">
            <div class="reference-set-intro" id="reference-set-hint"><strong>3 required · up to 5 total</strong><span>CreatorPilot fetches each transcript automatically and analyzes every video separately.</span></div>
            ${referenceRows(draft)}
      ...
      <div class="form-submit"><a class="button button-secondary" href="#/dashboard">Cancel</a><button class="button button-primary" type="submit">Fetch & analyze references ${icon("arrow")}</button></div>
    </form>`;
}
